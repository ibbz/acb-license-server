...
const POSITION_LAYOUTS = {
    'before-heading': ['band'],
    'after-intro':    ['inline-left', 'inline-right'],
    'end-of-section': ['band', 'inline-left', 'inline-right'],
};
...
- PREFER the inline layouts. "inline-left" and "inline-right" are narrow images that body text wraps around, and they read far better inside an article than a full-width band does — use them with "after-intro" or "end-of-section".
- Use "band" (full width) SPARINGLY: at most one per article, and only when the image genuinely deserves the whole column, such as a wide scene or a before/after. Pair it with "before-heading" or "end-of-section".
- Alternate "inline-left" and "inline-right" if you suggest more than one inline image.
- Never pair "before-heading" with an inline layout — a floated image above a heading has nothing to wrap against.
...
const SCHEMA_EXAMPLE = `[
  {
    ...
    "position": "before-heading | after-intro | end-of-section",
...
function normaliseSuggestion(raw, headings) {
    ...
    if (!POSITIONS.includes(position)) position = 'end-of-section';
...
module.exports = {
    SUGGESTIONS_MAX,
    POSITIONS, POSITION_LAYOUTS, LAYOUTS,
