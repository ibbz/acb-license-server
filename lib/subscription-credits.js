...
async function grantDueAnnualCredits(pool, { licenseKey = null } = {}) {
  ...
  for (const row of due.rows) {
    ...
    // Advance the next-grant date by a month, but ONLY if it still holds the
    // value we read. Optimistic guard: two concurrent sweeps (the daily cron and
    // a lazy /credits top-up, or two simultaneous /credits misses) can both read
    // the same due row; the grant above is dedupe-protected so it can't double,
    // but without this guard each would advance the date a month and the
    // subscriber would SKIP a month. The second writer now matches 0 rows and
    // no-ops. Never push past the term end.
    await pool.query(
      `UPDATE license_keys
       SET next_credit_grant_at = LEAST(next_credit_grant_at + INTERVAL '1 month',
           ...
           updated_at = NOW()
       WHERE id = $1 AND next_credit_grant_at = $2`,
      [row.id, row.next_credit_grant_at]
    );
    if (ok) granted++;
  }
  return granted;
...
// Mid-cycle UPGRADE grant. When a subscriber moves UP a tier between renewals,
// add the new tier's allowance ON TOP of their existing balance — nothing is
// zeroed, so unused monthly credits AND never-expiring bundle credits are both
// preserved. The note uses the 'subscription_credits:' family so the normal
// no-rollover renewal sweep clears it at the next cycle (the bonus is mid-cycle
// only). Idempotent per dedupeKey, so duplicate subscription.updated events for
// one upgrade — and upgrade/downgrade churn within the same period — can't re-add.
async function grantUpgradeCredits(pool, { licenseId, tier, dedupeKey, expiresInDays = 35 }) {
  const allowance = CREDIT_ALLOWANCE[(tier || '').toLowerCase()];
  if (!allowance) {
    console.log(`[sub-credits] No credit allowance for tier '${tier}' — skipping upgrade grant`);
    return false;
  }
  const note = `subscription_credits:upgrade:${dedupeKey}`;
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    if (dup.rows.length > 0) {
      await client.query('ROLLBACK');
      console.log(`[sub-credits] Duplicate upgrade grant ignored: ${note}`);
      return false;
    }

    // Additive — no zeroing. The existing balance (monthly remainder + bundles)
    // is left intact and this allowance stacks on top until the next renewal.
    await client.query(
      `INSERT INTO credit_batches
         (license_key_id, credits_issued, credits_remaining, source, issued_date, expiry_date, notes)
       VALUES ($1, $2, $2, 'subscription', CURRENT_DATE, CURRENT_DATE + ($3::int * INTERVAL '1 day'), $4)`,
    ...
    await client.query('COMMIT');
    console.log(`[sub-credits] Upgrade grant: +${allowance} '${tier}' credits to license_id=${licenseId} (${note})`);
    return true;
  } catch (err) {
    await client.query('ROLLBACK');
    console.error('[sub-credits] grantUpgradeCredits failed:', err.message);
    throw err;
  } finally {
    client.release();
  }
...
module.exports = { grantSubscriptionCredits, grantUpgradeCredits, grantDueAnnualCredits };
