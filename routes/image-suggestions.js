...
function dedupeAndCap(suggestions, max) {
    ...
    for (const s of suggestions) {
        ...
        // inline image; if its position could not legally carry an inline layout
        // it is reset to DEFAULT_POSITION, which can. (This previously said the
        // demotion moved to 'end-of-section' — that position no longer exists,
        // and the code has always used DEFAULT_POSITION.)
        if (s.layout === 'band') {
            if (bandCount >= 1) {
